...
def write_all_coor(iiith, d_huts,d_walls,townhall):
    xco = []
    yco = []
...
def check_neigh(troopsee, iiith):
    ran = -1
...
def attack_troops(troopsee, iiith, d_huts,d_walls,d_all,townhall):
    ran = -1
    nodes = write_all_coor(iiith,d_huts,d_walls,townhall)
    node = np.array([[troopsee.get_posx(), troopsee.get_posy()]]).T
    x_coor = [1, 1, -1, -1, 1, 0, -1, 0]
    y_coor = [1, -1, 1, -1, 0, 1, 0, -1]
    # letter = ["RU","RD","LU","LD","R","U","L","D"]
    ...
    for i in range(0, 8):
        ...
        if(len(result[0]) != 0):
          if(iiith.get_char(arr3[0][i], arr3[1][i]) == 'H'):
            # troopsee.change_attackmode()
            bb=[key for key, value in d_huts.items() if value[0] == arr3[0][i] and value[1]==arr3[1][i]]
            ran = (bb[0])
...
def final_step(arr,iiith, king,d_huts,d_walls,d_troops,d_all,townhall,a_huts,a_walls,a_troops,d_all2):
    for key,value in d_troops.items():
        i=key
        if(a_troops[i].get_destroy()!=1):
          var=a_troops[i]
          var2 = time.time() - (var.get_starttime() + var.get_steps())
        #   print(var2)
          if(var2 >= var.get_speed() and var2< 2*(var.get_speed())):
            var=a_troops[i]
            result=attack_troops(var,iiith,d_huts,d_walls,d_all,townhall)
            # print(result)
            ...
            if(var.get_destroy()==0): 
             iiith.objectc(var)
          check_neigh(var, iiith)    

def Troops_movement(iiith, king,d_huts,d_walls,d_troops,d_all,townhall,a_huts,a_walls,a_troops,d_all2):
    nodes = write_all_coor(iiith, d_huts,d_walls,townhall)
    arr = {}
    for key,value in d_troops.items():
        ...
        # print(tempo3)
        arr[key]=tempo3
    final_step(arr,iiith, king,d_huts,d_walls,d_troops,d_all,townhall,a_huts,a_walls,a_troops,d_all2)
