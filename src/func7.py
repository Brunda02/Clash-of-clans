...
def write_all_cooraa2(iiith, d_huts,d_walls,townhall,d_all3):
    xco = []
    yco = []
...
def write_all_cooraa3(iiith, d_huts,d_walls,d_all,townhall,d_all3):
    xco = []
    yco = []
...
def check_neigh2(troopsee, iiith):
    ...
    for i in range(0, 6):
        for j in range(0,6):
         if(node[0]+i >=1 and node[0]+i <=28 and node[1]+j >=0 and node[1]+j <=89):   
...
def attack_archers2(troopsee, iiith, d_huts,d_walls,d_all,townhall,d_all3):
    # print("y")
    ran = -1
    nodes = write_all_cooraa3(iiith, d_huts,d_walls,d_all,townhall,d_all3)
    ...
    for key,value in d_all.items():
        ...
        if( tempuu<=6):
            # mini=tempuu
            arra2=tempuu2
            ff=(value[0]-arra2[0])*(value[0]-arra2[0])+(value[1]-arra2[1])*(value[1]-arra2[1])
            if(ff<mini):
                mini=ff
                ran=key
    for key,value in d_all3.items():
    ...
    # bb3 = [i for i in bb2 if i <= 13 and i>=22]
    return ran




def final_step2(arr,iiith, king,d_huts,d_walls,d_archers,d_all,townhall,a_huts,a_walls,a_archers,d_all3,d_towers,d_cannons,a_all3,d_all2):
    for key,value in d_archers.items():
        ...
        #   print(var2)
          if(var2 >= var.get_speed() and var2< 2*(var.get_speed())):
            var=a_archers[i]
            result=attack_archers2(var,iiith,d_huts,d_walls,d_all,townhall,d_all3)
            # print(result)
...
def Archers_movement2(iiith, king,d_huts,d_walls,d_archers,d_all,townhall,a_huts,a_walls,a_archers,d_all3,d_towers,d_cannons,a_all3,d_all2):
    nodes = write_all_cooraa2(iiith, d_huts,d_walls,townhall,d_all3)
    arr = {}
    # print(len(d_archers))
    for key,value in d_archers.items():
        ...
        arr[key]=tempo3
        # print(arr[key])
    final_step2(arr,iiith, king,d_huts,d_walls,d_archers,d_all,townhall,a_huts,a_walls,a_archers,d_all3,d_towers,d_cannons,a_all3,d_all2)
